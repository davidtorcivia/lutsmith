...
from lutsmith.config import MAX_CUBE_SIZE, MAX_CUBE_FILE_LINES
from lutsmith.errors import LUTFormatError, ExportError
...
def write_cube(
    ...
    if shaper is not None:
        if shaper.ndim != 2 or shaper.shape[1] != 3:
            raise ExportError(
                f"Shaper must have shape (M, 3), got {shaper.shape}"
            )
        if shaper.shape[0] < 2:
            raise ExportError("Shaper LUT must have at least 2 entries")

    lines = []
    if title:
        lines.append(f'TITLE "{title}"')
    lines.append("# Generated by LutSmith")
    lines.append(f"# Optimized for {kernel_hint} interpolation")
    lines.append("")

    if shaper is not None:
        lines.append(f"LUT_1D_SIZE {shaper.shape[0]}")
...
def read_cube(filepath: str | Path) -> tuple[np.ndarray, dict]:
    ...
    with open(path, "r") as f:
        for raw_line in f:
            ...
            if upper.startswith("TITLE"):
                ...
                if reading_1d and size_1d is not None and len(values_1d) < size_1d:
                    raise LUTFormatError("Incomplete LUT_1D data before LUT_3D_SIZE")
                try:
                ...
                try:
                    ...
                    raise LUTFormatError(f"Invalid LUT_1D_SIZE: {line}")
                if size_1d < 2:
                    raise LUTFormatError(f"LUT_1D_SIZE {size_1d} out of range [2, inf)")
                reading_1d = True
                continue
                ...
                if len(parts) >= 3:
                    ...
                    if reading_1d and size_1d is not None and len(values_1d) < size_1d:
                        values_1d.append(triplet)
                        if len(values_1d) >= size_1d:
                            reading_1d = False
                    else:
    ...
    metadata = {
        ...
        "size": size,
        "shaper_1d": np.array(values_1d, dtype=np.float32) if values_1d else None,
    }
