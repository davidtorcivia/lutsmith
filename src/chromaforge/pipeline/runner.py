...
from chromaforge.core.lut import create_lut_data
...
def run_pipeline(
    ...
    source, target, meta = preprocess_pair(
    ...
    # Compute weights
    weights = compute_sample_weights(bins, config)
    ...
    lut_array, solver_infos = solve_lut(
        input_rgb, output_rgb, alpha, config, occupied_indices
    )
    ...
    if config.enable_refinement:
        ...
        lut_array, refine_diag = refine_lut(
            lut_array, input_rgb, output_rgb, alpha,
            config, occupied_indices,
    ...
    if config.generate_coverage_report:
        coverage_map = compute_coverage_map(occupied_indices, config.lut_size)
